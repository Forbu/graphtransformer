...
class NodeFeatureAggregator(nn.Module):
    ...
    def forward(self, node_features: Tensor) -> Tensor:
        ...
        aggregated_features = torch.hstack((mean_features, min_features, max_features, std_features))
...
class GraphAttentionBlock(nn.Module):
    ...
    """
    def __init__(self, node_dim: int, edge_dim: int, global_dim: int, num_heads: int, **kwargs):
        super().__init__()
        assert node_dim % num_heads == 0, f"Node dimension ({node_dim}) must be divisible by number of heads ({num_heads})."
        ...
        self.edge_output_mlp = nn.Linear(node_dim, edge_dim) # From intermediate node_dim back to edge_dim
        self.global_output_mlp = nn.Sequential(nn.Linear(global_dim, global_dim), nn.ReLU(), nn.Linear(global_dim, global_dim))
    ...
    def forward(self, X: Tensor, E: Tensor, y: Tensor, node_mask: Tensor):
        ...
        Q = (self.q_proj(X) * node_mask_expanded).view(batch_size, num_nodes, self.num_heads, self.head_dim)
        K = (self.k_proj(X) * node_mask_expanded).view(batch_size, num_nodes, self.num_heads, self.head_dim)
        V = (self.v_proj(X) * node_mask_expanded).view(batch_size, num_nodes, self.num_heads, self.head_dim)
        ...
        raw_attention_scores = (Q.unsqueeze(2) * K.unsqueeze(1)) / math.sqrt(self.head_dim)
        
        # Incorporate edge features into attention scores using FiLM
        edge_factor_mul = self.edge_to_attn_mul(E).view(batch_size, num_nodes, num_nodes, self.num_heads, self.head_dim)
        edge_factor_add = self.edge_to_attn_add(E).view(batch_size, num_nodes, num_nodes, self.num_heads, self.head_dim)
        
        # The new attention "logits" are modulated by edge features
        # Shape: (bs, n, n, n_head, d_head)
        attention_logits = raw_attention_scores * (edge_factor_mul + 1) + edge_factor_add
        
        # 3. Update Edge Features
        # The attention logits themselves form the basis for the new edge representation
        intermediate_edge_repr = attention_logits.flatten(start_dim=3) # (bs, n, n, node_dim)
        
        # Modulate with global features
        global_to_edge_add = self.global_to_edge_add(y).unsqueeze(1).unsqueeze(1) # (bs, 1, 1, node_dim)
        global_to_edge_mul = self.global_to_edge_mul(y).unsqueeze(1).unsqueeze(1) # (bs, 1, 1, node_dim)
        intermediate_edge_repr = global_to_edge_add + (global_to_edge_mul + 1) * intermediate_edge_repr
        ...
        attention_logits_summed = attention_logits.sum(dim=-1) # (bs, n, n, n_head)
        
        softmax_mask = edge_mask_col.expand(-1, num_nodes, -1, self.num_heads) # (bs, n, n, n_head)
        attention_weights = masked_softmax(attention_logits_summed, softmax_mask, dim=2) # (bs, n, n, n_head)
        
        # Aggregate values based on attention weights
        # attn: (bs, n, n, n_head), V: (bs, 1, n, n_head, d_head) -> weighted_V: (bs, n, n_head, d_head)
        weighted_V = torch.einsum('bijn,bjnh->binh', attention_weights, V.transpose(1, 2))
        weighted_V = weighted_V.reshape(batch_size, num_nodes, self.node_dim) # (bs, n, node_dim)

        # Modulate with global features
        global_to_node_add = self.global_to_node_add(y).unsqueeze(1)
        global_to_node_mul = self.global_to_node_mul(y).unsqueeze(1)
        updated_X_intermediate = global_to_node_add + (global_to_node_mul + 1) * weighted_V
...
class GraphTransformerLayer(nn.Module):
    ...
    """
    def __init__(self, node_dim: int, edge_dim: int, global_dim: int, num_heads: int,
                 node_ff_dim: int = 2048, edge_ff_dim: int = 128, global_ff_dim: int = 2048,
                 dropout: float = 0.1, layer_norm_eps: float = 1e-5, **kwargs) -> None:
        super().__init__()

        self.attention_block = GraphAttentionBlock(node_dim, edge_dim, global_dim, num_heads, **kwargs)
        ...
        self.ffn_node = nn.Sequential(
            nn.Linear(node_dim, node_ff_dim), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(node_ff_dim, node_dim), nn.Dropout(dropout)
        )
        ...
        self.ffn_edge = nn.Sequential(
            nn.Linear(edge_dim, edge_ff_dim), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(edge_ff_dim, edge_dim), nn.Dropout(dropout)
        )
        ...
        self.ffn_global = nn.Sequential(
            nn.Linear(global_dim, global_ff_dim), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(global_ff_dim, global_dim), nn.Dropout(dropout)
        )
    ...
    def forward(self, X: Tensor, E: Tensor, y: Tensor, node_mask: Tensor):
        ...
        attn_out_X, attn_out_E, attn_out_y = self.attention_block(X, E, y, node_mask=node_mask)
...
class PlaceHolder:
    ...
    def mask(self, node_mask):
        ...
        self.E = self.E * node_mask.unsqueeze(1).unsqueeze(-1) * node_mask.unsqueeze(2).unsqueeze(-1)
        return self
...
class GraphTransformer(nn.Module):
    ...
    hidden dimensions.
    """
    def __init__(self, num_layers: int, input_dims: dict, hidden_dims: dict,
                 output_dims: dict, act_fn_in: nn.Module, act_fn_out: nn.Module):
        super().__init__()
        ...
        self.output_mlp_X = nn.Sequential(
            nn.Linear(hidden_dims['node_dim'], hidden_dims['node_dim']), act_fn_out,
            nn.Linear(hidden_dims['node_dim'], output_dims['X']))
        self.output_mlp_E = nn.Sequential(
            nn.Linear(hidden_dims['edge_dim'], hidden_dims['edge_dim']), act_fn_out,
            nn.Linear(hidden_dims['edge_dim'], output_dims['E']))
        self.output_mlp_y = nn.Sequential(
            nn.Linear(hidden_dims['global_dim'], hidden_dims['global_dim']), act_fn_out,
    ...
    def forward(self, X: Tensor, E: Tensor, y: Tensor, node_mask: Tensor):
        ...
        E_processed = E_processed * node_mask.unsqueeze(1).unsqueeze(-1) * node_mask.unsqueeze(2).unsqueeze(-1)

        # 2. Pass through the transformer layers
        for layer in self.transformer_layers:
            X_processed, E_processed, y_processed = layer(X_processed, E_processed, y_processed, node_mask)
        ...
        E_final = (E_out + E_residual) * diag_mask # Use mask to prevent self-loops in residual
